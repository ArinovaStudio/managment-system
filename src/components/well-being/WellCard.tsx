...
function WellCard({ data, index }: { data?: any, index?: number }) {
  ...
  return (
        <div className={`h-full w-full ${index === 0 ? "mt-4" : "mt-0"}`}>
          <div className="w-full h-auto min-h-40 p-4 pb-12 relative rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
           <div className="absolute right-6 -top-2 bg-purple-400/20 text-purple-400 backdrop-blur-sm rounded-full text-sm py-0.5 px-4"> {data?.category || 'Tips'}</div>
          ...
          <div className="w-full px-4 absolute bottom-0 left-0 flex justify-end items-end gap-3 pt-4">
            <button 
